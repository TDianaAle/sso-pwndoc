...
//-----------------oidc------------------------------------------------------//
    // OIDC Login - Inizialize authentication with Keycloak
    app.get('/api/users/oidc/login', function(req, res, next) {
        console.log('=== CALL TO OIDC LOGIN ===');
        next();
    }, passport.authenticate('oidc'));

    // OIDC Callback - Keycloak sends user here after authentication
    app.get('/api/users/oidc/callback', function(req, res, next) {
        console.log('=== CALLBACK ROUTE ACHIEVED ===');
        ...
        passport.authenticate('oidc', function(err, user, info) {
            ...
            if (err) {
                console.log('ERROR PASSPORT:', err.message);
            ...
            req.logIn(user, function(loginErr) {
                if (loginErr) {
                    console.log('ERROR LOGIN:', loginErr);
                ...
                .catch(tokenErr => {
                    console.log('ERROR TOKEN:', tokenErr);
